...
Release 由仓库 owner 在默认分支通过 `workflow_dispatch`，或通过 `repository_dispatch`（event type `release`）启动。请求中的 annotated `v*` tag 必须匹配 package 版本且指向 `origin/main` 历史；只读 job 构建 `CodePath.zip`，随后 `release` environment 中的独立写权限 job 发布资产。README 的用户下载链接必须指向这个固定资产名。
...
- 功能稳定后再合并到 `main`；正式发布时创建并推送 annotated `v*` tag，再由仓库 owner 运行 Release workflow 或发送受信 `release` repository dispatch，让受控 workflow 生成 `CodePath.zip`。
